...
import { PublicSiteShell, type PublicDiscoveryKey } from "@/components/marketing/public-site-shell";
...
export function MarketplaceListPage({ viewModel }: { viewModel: MarketplaceListViewModel }) {
  return (
    <PublicSiteShell active="discover" discoveryActive={getDiscoveryActive(viewModel)}>
      <div className="marketplace-page">
        ...
        <section className="marketplace-section" aria-labelledby="marketplace-results-title">
          <div className="vy-container marketplace-results">
            <div className="marketplace-results__main">
              <MarketplacePrimaryContent viewModel={viewModel} />
            </div>
            <aside className="marketplace-sidebar" aria-label="Refinar descoberta">
              {viewModel.focus !== "categories" ? (
                <MarketplaceFacetSection
                  title="Categorias"
                  items={viewModel.categories.map((category) => ({
                    href: `/categorias/${category.slug}`,
                    label: category.name,
                    meta: formatBusinessCount(category.businessCount)
                  }))}
                />
              ) : null}
              {viewModel.focus !== "cities" ? (
                <MarketplaceFacetSection
                  title="Locais"
                  items={viewModel.cities.map((city) => ({
                    href: `/locais/${city.slug}`,
                    label: city.name,
                    meta: formatBusinessCount(city.businessCount)
                  }))}
                />
              ) : null}
            </aside>
        ...
        {viewModel.focus !== "offers" ? <OffersBand offers={viewModel.offers} /> : null}
      </div>
    </PublicSiteShell>
  );
}

function MarketplacePrimaryContent({ viewModel }: { viewModel: MarketplaceListViewModel }) {
  if (viewModel.focus === "offers") {
    return (
      <>
        <SectionHeader
          eyebrow="Promoções"
          title={
            viewModel.offers.length > 0
              ? `${viewModel.offers.length.toLocaleString("pt-MZ")} ofertas disponíveis`
              : "Ainda não existem ofertas públicas ativas"
          }
          description="Campanhas publicadas pelos negócios, com imagem, validade e estabelecimento identificado."
        ...
        {viewModel.offers.length > 0 ? (
          ...
          <MarketplaceEmptyState kind="offers" />
        )}
      </>
    );
  }

  if (viewModel.focus === "categories") {
    return (
      <>
        <SectionHeader
          eyebrow="Categorias"
          title="Escolha o que procura"
          description="Navegue por tipo de negócio e veja quantos estabelecimentos e ofertas estão disponíveis."
          id="marketplace-results-title"
        />
        {viewModel.categories.length > 0 ? (
          <div className="marketplace-discovery-grid">
            {viewModel.categories.map((category) => (
              <Link
                className="marketplace-discovery-card"
                href={`/categorias/${category.slug}`}
                key={category.id}
              >
                ...
                <span>
                  ...
                  <small>
                    {formatBusinessCount(category.businessCount)} ·{" "}
                    {formatOfferCount(category.offerCount)}
                  </small>
                </span>
          ...
          <MarketplaceEmptyState kind="categories" />
        )}
      </>
    );
  }

  if (viewModel.focus === "cities") {
    return (
      <>
        <SectionHeader
          eyebrow="Locais"
          title="Descubra benefícios perto de si"
          description="Abra uma cidade para encontrar estabelecimentos, categorias e ofertas publicadas nessa zona."
          id="marketplace-results-title"
        />
        {viewModel.cities.length > 0 ? (
          <div className="marketplace-discovery-grid">
            {viewModel.cities.map((city) => (
              ...
              >
                ...
                <span>
                  ...
                  <small>
                    {formatBusinessCount(city.businessCount)} · {formatOfferCount(city.offerCount)}
                  </small>
                </span>
                <ArrowRight aria-hidden="true" size={18} />
              </Link>
          ...
          <MarketplaceEmptyState kind="cities" />
        )}
      </>
    );
  }
      ...
      <SectionHeader
        ...
        title={
          viewModel.businesses.length > 0
            ...
            : "Ainda sem estabelecimentos publicados"
        }
        description="Cada perfil mostra YELAS, benefícios, localização e ofertas públicas ativas."
        id="marketplace-results-title"
      />
      {viewModel.businesses.length > 0 ? (
        ...
        <MarketplaceEmptyState kind="businesses" />
      )}
    </>
  );
}
...
export function BusinessDetailPage({ viewModel }: { viewModel: MarketplaceDetailViewModel }) {
  ...
  return (
    <PublicSiteShell active="discover" discoveryActive="establishments">
      <div className="marketplace-page">
        <MarketplaceHero
            ...
            <article className="marketplace-detail">
              ...
              <DetailSection title="Perguntas frequentes">
                <div className="marketplace-faq">
                  ...
                  <details>
                    <summary>As YELAS podem ser levantadas em dinheiro?</summary>
                    <p>
...
export function OfferDetailPage({ viewModel }: { viewModel: MarketplaceOfferViewModel }) {
  return (
    <PublicSiteShell active="discover" discoveryActive="offers">
      <div className="marketplace-page">
        <MarketplaceHero
          eyebrow="Oferta ativa"
          title={viewModel.offer.title}
          description={`Oferta pública de ${viewModel.business.name}. Consulte os detalhes, o estabelecimento e a validade da promoção.`}
          breadcrumbs={viewModel.breadcrumbs}
...
export function MarketplaceSearchPage({ viewModel }: { viewModel: MarketplaceSearchViewModel }) {
  ...
  return (
    <PublicSiteShell active="discover" discoveryActive="search">
      <div className="marketplace-page">
        <MarketplaceHero
      ...
      <Link href={href}>
        <span className="marketplace-offer-card__media">
          {offer.imageUrl ? (
            ...
            <span className="marketplace-offer-card__placeholder" aria-hidden="true">
              <Gift size={28} />
              VUYELA
            </span>
          )}
        </span>
        <span className="marketplace-offer-card__category">
          <Tag size={16} />
...
function MarketplaceEmptyState({
  ...
  kind: "businesses" | "categories" | "cities" | "offers";
}) {
  const content = {
    businesses: {
      title: "Conteúdo público em preparação",
      body: "Os novos estabelecimentos surgirão aqui depois de aprovados e publicados.",
      href: "/cadastrar/negocio",
      action: "Registar negócio"
    },
    categories: {
      title: "Categorias em preparação",
      body: "As categorias ficam disponíveis assim que tiverem estabelecimentos públicos associados.",
      href: "/estabelecimentos",
      action: "Ver estabelecimentos"
    },
    cities: {
      title: "Locais em preparação",
      body: "Os locais aparecem quando uma filial ativa tem uma cidade publicada.",
      href: "/estabelecimentos",
      action: "Ver estabelecimentos"
    },
    offers: {
      title: "Ainda não existem ofertas públicas ativas",
      body: "Explore os estabelecimentos disponíveis enquanto os negócios preparam novas promoções.",
      href: "/estabelecimentos",
      action: "Explorar estabelecimentos"
    }
  }[kind];
  ...
  return (
    <div className="marketplace-empty">
      ...
      <h2>{content.title}</h2>
      <p>{content.body}</p>
      <Link className="marketplace-button marketplace-button--reward" href={content.href}>
        {content.action}
      </Link>
    </div>
  );
...
function getDiscoveryActive(viewModel: MarketplaceListViewModel): PublicDiscoveryKey {
  if (viewModel.canonicalPath.startsWith("/categorias")) {
    return "categories";
  }

  if (viewModel.canonicalPath.startsWith("/locais")) {
    return "cities";
  }

  if (viewModel.canonicalPath.startsWith("/ofertas")) {
    return "offers";
  }
...
function formatBusinessCount(value: number): string {
  return `${value.toLocaleString("pt-MZ")} ${value === 1 ? "negócio" : "negócios"}`;
}

function formatOfferCount(value: number): string {
  return `${value.toLocaleString("pt-MZ")} ${value === 1 ? "oferta" : "ofertas"}`;
}
