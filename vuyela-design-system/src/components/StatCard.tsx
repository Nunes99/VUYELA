...
export function StatCard({
  ...
  return (
    <article className={["vy-stat", className].filter(Boolean).join(" ")}>
      <div className="vy-stat__header">
        <span className="vy-stat__label">{label}</span>
        {icon ? (
          <span className="vy-stat__icon" aria-hidden="true">
            {icon}
          </span>
        ) : null}
      </div>
      ...
      {delta ? (
        <span className={`vy-stat__delta vy-stat__delta--${direction}`}>
